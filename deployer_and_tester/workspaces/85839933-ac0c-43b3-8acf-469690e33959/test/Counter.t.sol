...
contract CounterTest is Test {
    ...
    function testBeforeSwap() public {
        ...
        (bytes4 hookReturn, BeforeSwapDelta memory delta, uint24 fee) = counter._beforeSwap(address(this), poolKey, swapParams, "");
        assertEq(hookReturn, Counter._beforeSwap.selector);
    ...
    function testAfterSwap() public {
        ...
        (bytes4 hookReturn, int128 feeGrowthDelta) = counter._afterSwap(address(this), poolKey, swapParams, balanceDelta, "");
        assertEq(hookReturn, Counter._afterSwap.selector);
        assertEq(feeGrowthDelta, 0);
    ...
    function testBeforeAddLiquidity() public {
        ...
        bytes4 hookReturn = counter._beforeAddLiquidity(address(this), poolKey, modifyLiquidityParams, "");
        assertEq(hookReturn, Counter._beforeAddLiquidity.selector);
        
        uint256 count = counter.beforeAddLiquidityCount(poolKey.toId());
        assertEq(count, 1);
    }
    ...
    function testBeforeRemoveLiquidity() public {
        ...
        bytes4 hookReturn = counter._beforeRemoveLiquidity(address(this), poolKey, modifyLiquidityParams, "");
        assertEq(hookReturn, Counter._beforeRemoveLiquidity.selector);
        
        uint256 count = counter.beforeRemoveLiquidityCount(poolKey.toId());
        assertEq(count, 1);
    }
}