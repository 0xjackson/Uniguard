...
contract CounterTest is Test {
    ...
    function testBeforeSwap() public {
        ...
        (bytes4 hookResponse, BeforeSwapDelta delta, uint24 fee) = counter._beforeSwap(address(this), poolKey, swapParams, hookData);
        assertEq(hookResponse, Counter._beforeSwap.selector);
    ...
    function testAfterSwap() public {
        ...
        bytes memory hookData = bytes("");
        (bytes4 hookResponse, int128 amountOut) = counter._afterSwap(address(this), poolKey, swapParams, balanceDelta, hookData);
        assertEq(hookResponse, Counter._afterSwap.selector);
    ...
    function testBeforeAddLiquidity() public {
        ...
        bytes memory hookData = bytes("");
        bytes4 hookResponse = counter._beforeAddLiquidity(address(this), poolKey, modifyLiquidityParams, hookData);
        assertEq(hookResponse, Counter._beforeAddLiquidity.selector);
    ...
    function testBeforeRemoveLiquidity() public {
        ...
        bytes memory hookData = bytes("");
        bytes4 hookResponse = counter._beforeRemoveLiquidity(address(this), poolKey, modifyLiquidityParams, hookData);
        assertEq(hookResponse, Counter._beforeRemoveLiquidity.selector);
